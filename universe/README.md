...
be bought in the target account, ranked by index weight, with the untradeable tail
...
```bash
python3 build_universe.py t212_instruments.json iwm_holdings.json eligible-universe.md
```
...
Trading 212's `ticker` field preserves the symbol from the day the instrument was
added, and goes stale after a rename or a SPAC merger:

| Russell says | Broker ticker | Currently displays as |
|---|---|---|
